...
ps5 #148 taxa and 171 samples
ps5_ra<-transform_sample_counts(ps5, function(OTU) OTU/sum(OTU))
#figure out how many colors you need
get_taxa_unique(ps5_ra, "Order") #59
get_taxa_unique(ps5_ra, "Class") #33
get_taxa_unique(ps5_ra, "Genus") #88
...
pdf("BarChart_DCYL_location.pdf", width=10, height=8)
...
pdf("BarChart_DCYL_reef.pdf", width=10, height=8)
p2=plot_bar(ps5_ra, fill="Class")+
  geom_bar(aes(fill=Class), stat="identity",position="stack") +
  theme(strip.text=element_text(face="bold"))+
  theme(axis.text.x=element_blank())+
  scale_fill_manual(values=palette)+
